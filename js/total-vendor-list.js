        ...
        vendors.forEach(vendor => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    <button class="delete-btn" onclick="openDeleteVendorModal(${vendor.id})">Delete</button>
                </td>
            `;
...
checkAuth('agent').then(isAuthenticated => {
    if (isAuthenticated) {
        fetchAgentId().then(success => {
            if (success) {
                renderMenu(['order-placement','agent-dashboard']);
                setDefaultFilters();
                fetchVendors();
            }
        });
